...
/// Copy every skill bundled with the marketplace plugins into
/// `<project_dir>/.claude/skills/`.
///
/// The embedded tree is `plugins/<plugin>/skills/<skill>/`; the plugin level is
/// flattened away so the generated project sees a plain `skills/<skill>/`.
pub fn copy_skills(embedded_plugins: &Dir, project_dir: &Path) -> io::Result<()> {
    let skills_dest = project_dir.join(".claude").join("skills");

    for plugin in embedded_plugins.dirs() {
        let Some(skills_dir) = plugin.dirs().find(|d| {
            d.path().file_name().and_then(|n| n.to_str()) == Some("skills")
        }) else {
            continue;
        };

        for skill in skills_dir.dirs() {
            let skill_name = skill
                .path()
                .file_name()
                .ok_or_else(|| io::Error::new(io::ErrorKind::Other, "invalid embedded skill path"))?;
            extract_dir(skill, &skills_dest.join(skill_name))?;
        }
    }
